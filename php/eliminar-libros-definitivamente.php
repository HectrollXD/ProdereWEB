    ...
    if( (!empty($_POST)) && isset($_SESSION['usuario']) ){
        ...
        if(empty($busqueda)){
            ...
            while($rows = oci_fetch_array($Query, OCI_NUM + OCI_RETURN_NULLS)){
                ...
                $tabla.="
                        <tr>
                            ...
                            <td>$rows[3]</td>
                            <td>$rows[2]</td>
                            <td><button class='btn btn-danger' value='$rows[0]'>Eliminar</button></td>
                        </tr>
                ";
            }
            $tabla.='
            ...
            if(oci_num_rows($Query) === 0){
                $tabla = "No hay libros por eliminar.";
            }
        }
        else{
            $Consulta = "SELECT * FROM LIBROS WHERE ELIMINADO = 1 AND (CODIGO_DE_LIBRO LIKE '%$busqueda%' OR TITULO LIKE '%$busqueda%' OR EDITORIAL LIKE '%$busqueda%')";
            $Query = oci_parse($Conexion_a_la_base_de_datos_Oracle, $Consulta);
            oci_execute($Query);
            while($rows = oci_fetch_array($Query, OCI_NUM + OCI_RETURN_NULLS)){
                $titulo = ucwords(strtolower($rows[1]));
