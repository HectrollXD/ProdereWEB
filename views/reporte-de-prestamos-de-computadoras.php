<?php
    ...
    $pdf -> SetTitle('Reporte de prestamos de computadoras.');
    ...
    $pdf -> Cell(336, 12, 'Reporte de registro de prestamos de computadoras.', 1, 1, 'C', 1);
    $pdf -> SetFont('Helvetica','B',11);
    $pdf -> SetTextColor(0,0,0);
    $pdf -> SetFillColor(170,170,170);
    $pdf -> SetXY($pdf->GetX()+10, $pdf->GetY());
    $pdf -> Cell(25, 7, '#', 1, 0, 'C', 1);
    $pdf -> Cell(35, 7, 'No. Computadora', 1, 0, 'C', 1);
    $pdf -> Cell(35, 7, 'Código', 1, 0, 'C', 1);
    $pdf -> Cell(100, 7, 'Nombre del alumno', 1, 0, 'C', 1);
    $pdf -> Cell(33, 7, 'Fecha', 1, 0, 'C', 1);
    $pdf -> Cell(20, 7, 'Entrada', 1, 0, 'C', 1);
    $pdf -> Cell(20, 7, 'Salida', 1, 0, 'C', 1);
    $pdf -> Cell(40, 7, 'Firma', 1, 0, 'C', 1);
    $pdf -> Cell(28, 7, 'Status', 1, 1, 'C', 1);
    $Select = "SELECT * FROM PRESTAMOS_DE_COMPUTADORAS ORDER BY NUMERO_DE_PRESTAMO_DE_COMPU ASC";
    ...
    while($rows = oci_fetch_array($Query, OCI_NUM + OCI_RETURN_NULLS + OCI_RETURN_LOBS)){
        $nombre = ucwords(strtolower($rows[3]));
        if($rows[8] == 1){ $status = 'Entregado'; }
        else{ $status = 'N/A'; }
        if($rows[6] != ''){ $salida = $rows[6]; }
        else{ $salida = 'N/A'; }
        ...
        $pdf -> MultiCell(25, 15, $rows[0], 1, 'C', 0, 0, '', '', 1, 0, 0, 1, 15, 'M');
        $pdf -> MultiCell(35, 15, $rows[1], 1, 'C', 0, 0, '', '', 1, 0, 0, 1, 15, 'M');
        $pdf -> MultiCell(35, 15, $rows[2], 1, 'C', 0, 0, '', '', 1, 0, 0, 1, 15, 'M');
        $pdf -> MultiCell(100, 15, $nombre, 1, 'C', 0, 0, '', '', 1, 0, 0, 1, 15, 'M');
        $pdf -> MultiCell(33, 15, $rows[4], 1, 'C', 0, 0, '', '', 1, 0, 0, 1, 15, 'M');
        $pdf -> MultiCell(20, 15, $rows[5], 1, 'C', 0, 0, '', '', 1, 0, 0, 1, 15, 'M');
        ...
        $pdf -> SetFont('Helvetica','',10);
        $pdf -> MultiCell(28, 15, $status, 1, 'C', 0, 1, '', '', 1, 0, 0, 1, 15, 'M');
    }
    $pdf -> Output('Reporte de prestamos de computadoras.');
